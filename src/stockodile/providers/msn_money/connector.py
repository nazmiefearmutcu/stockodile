    ...
    async def backfill(
        ...
        try:
            ...
            if channel in ("bar", "ohlcv"):
                ...
                if duration_ns <= 5 * 24 * 60 * 60 * 1_000_000_000:
                    chart_type = "5D"
                ...
                async with self.session.get(url, params=params, headers=headers) as resp:
                    if resp.status == 200:
                        ...
                        if data and isinstance(data, list) and "series" in data[0]:
                            ...
                            for idx in range(n):
                                ...
                                bar = Bar(
                                    ...
                                    open=float(open_p[idx]) if idx < len(open_p) else 0.0,
                                    high=float(high_p[idx]) if idx < len(high_p) else 0.0,
                                    low=float(low_p[idx]) if idx < len(low_p) else 0.0,
                                    close=float(close_p[idx]) if idx < len(close_p) else 0.0,
                        ...
                        if data and isinstance(data, list):
                            ...
                            if last_split_factor and last_split_date:
                                ...
                                try:
                                    ...
                                    ts = int(dt.timestamp() * 1e9)
                                    if start_ns <= ts <= end_ns:
                                        val_str = str(last_split_factor).split(":")[0]
                                        val = float(val_str)
                                        yield CorporateAction(
                                            provider=self.name,
                                            symbol=symbol.upper(),
                ...
                if resp.status == 200:
                    ...
                    for stock_str in stocks:
                        try:
                            ...
                            if stock_data.get("RT00S", "").upper() == query_sym.upper():
